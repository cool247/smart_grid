  ...
  if (children) {
    return (
      <ListItemStyle ref={ref} open={open} activeRoot={active} onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave}>
        <NavItemContent icon={icon} title={title} children={children} activeRoot={active} />
      </ListItemStyle>
    );
  }
...
function NavItemContent({ icon, title, children, subItem, activeRoot }) {
  return (
    <>
      {icon && (
        <Box
          ...
          sx={{
            mr: 1,
            color: activeRoot ? (theme)=> theme.palette.primary.main :'action',
